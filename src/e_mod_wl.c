...
_pol_wl_tzsh_srv_del(Pol_Wl_Tzsh_Srv *tzsh_srv)
{
   ...
   ELOGF("TZSH",
         "SRV_DEL  |res:0x%08x|tzsh_srv:0x%08x|name:%s",
         tzsh ? tzsh->cp : NULL,
         tzsh ? tzsh->ec : NULL,
         (unsigned int)tzsh_srv->res_tzsh_srv,
         (unsigned int)tzsh_srv,
         tzsh_srv->name);
   ...
   if ((tzsh_client_ec) && (tzsh_srv_ec))
     {
        ELOGF("TZSH",
              ...
              NULL, NULL,
              (unsigned int)tzsh_client_ec,
              (unsigned int)tzsh_srv_ec);
...
_pol_wl_tzsh_client_add(Pol_Wl_Tzsh *tzsh, struct wl_resource *res_tzsh_client)
{
   ...
   ELOGF("TZSH",
         "CL_ADD   |res:0x%08x|tzsh_client:0x%08x",
         tzsh->cp, tzsh->ec,
...
_pol_wl_tzsh_client_del(Pol_Wl_Tzsh_Client *tzsh_client)
{
   ...
   ELOGF("TZSH",
         ...
         NULL, NULL,
         (unsigned int)tzsh_client->res_tzsh_client,
         (unsigned int)tzsh_client);
...
_pol_wl_e_client_is_valid(E_Client *ec)
{
   ...
   ELOGF("POLWL",
         "CHK_EC   |ec_valid:%d|ec_deleted:%d",
         ec->pixmap, ec, found, del);
   ...
   EINA_LIST_FOREACH(ec->comp_data->aux_hint.hints, l, hint)
     {
        ...
        if (!strcmp(hint->hint, supported_hints[0]))
          {
             if (!strcmp(hint->val, "1") && (ec->lock_client_location || ec->lock_client_size))
               {
                  ec->lock_client_location = EINA_FALSE;
                  ec->lock_client_size = EINA_FALSE;
                  send = EINA_TRUE;
               }
             else if (strcmp(hint->val, "1") && (!ec->lock_client_location || !ec->lock_client_size))
               {
...
static void
_tzpol_iface_cb_aux_hint_set(struct wl_client *client EINA_UNUSED, struct wl_resource *res_tzpol, struct wl_resource *surf, int32_t id, const char *name, const char *value)
{
   E_Pixmap *cp;
   Eina_List *hints;
   E_Comp_Wl_Client_Data *cdata;
   ...
   ELOGF("TZPOL", "HINT_SET |res_tzpol:0x%08x|id:%d, name:%s, val:%s, found:%d", NULL, NULL, res_tzpol, id, name, value, res);
}
...
static void
_tzpol_iface_cb_aux_hint_unset(struct wl_client *client EINA_UNUSED, struct wl_resource *res_tzpol, struct wl_resource *surf, int32_t id)
{
   E_Pixmap *cp;
   Eina_List *hints;
   E_Comp_Wl_Client_Data *cdata;
   ...
   ELOGF("TZPOL", "HINT_UNSET|res_tzpol:0x%08x|id:%d, found:%d", NULL, NULL, res_tzpol, id, res);
}
...
static const struct tizen_policy_interface _tzpol_iface =
{
   ...
   _tzpol_iface_cb_aux_hint_set,
   _tzpol_iface_cb_aux_hint_unset,
...
_tzpol_cb_unbind(struct wl_resource *res_tzpol)
{
   ...
   ELOGF("TZPOL",
         "UNBIND   |client:0x%08x|tzpol:0x%08x",
         NULL, NULL,
         (unsigned int)client,
         (unsigned int)tzpol);
...
_tzpol_cb_bind(struct wl_client *client, void *data EINA_UNUSED, uint32_t ver, uint32_t id)
{
   ...
   ELOGF("TZPOL",
         "BIND     |client:0x%08x|tzpol:0x%08x",
         NULL, NULL,
         (unsigned int)client,
         (unsigned int)tzpol);
...
_tzsh_cb_unbind(struct wl_resource *res_tzsh)
{
   ...
   client = wl_resource_get_client(res_tzsh);

   ELOGF("TZSH",
         "UNBIND   |client:0x%08x|tzsh:0x%08x",
         NULL, NULL,
         (unsigned int)client,
         (unsigned int)tzsh);
...
_tzsh_cb_bind(struct wl_client *client, void *data EINA_UNUSED, uint32_t ver, uint32_t id)
{
   ...
   ELOGF("TZSH",
         "BIND     |client:0x%08x|tzsh:0x%08x",
         NULL, NULL,
         (unsigned int)client,
         (unsigned int)tzsh);
