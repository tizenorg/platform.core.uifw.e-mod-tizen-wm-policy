        ...
        if (ec_vis)
          {
             ...
             if (show)
               {
                  zoom_x = ec->x + ((WIN_WIDTH + SCR_MARGIN_W) * progress);
                  zoom_y = ec->y + (157 * progress);

                  zoom = 1.0 - (0.3 * progress);
               }
             else
               {
                  zoom_x = ec->x + ((WIN_WIDTH + SCR_MARGIN_W) - ((WIN_WIDTH + SCR_MARGIN_W) * progress));
                  zoom_y = ec->y + (157 - (157 * progress));

                  zoom = 0.7 + (0.3 * progress);
               }

             evas_object_move(ec->frame, zoom_x, zoom_y);

             Evas_Map *map;
             map = evas_map_new(4);
             evas_map_util_points_populate_from_geometry(map, ec->client.x, ec->client.y, ec->client.w, ec->client.h, 0);
             evas_map_util_zoom(map, zoom, zoom, ec->w / 2, ec->h / 2);
