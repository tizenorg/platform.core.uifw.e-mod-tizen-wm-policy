   ...
   if (focused)
     {
        ...
        if (focused->indicator.state == 2) // state_on
          {
             if ((focused->indicator.opacity_mode == 3) &&
                 (focused->indicator.visible_type == 0)) // opacity: transparent, visible: hidden
               {
                  e_mod_pol_wl_indicator_flick_send(focused);
                  return EINA_TRUE;
               }
          }
        else if (focused->indicator.state == 1) // state_hide
          {
             return EINA_TRUE;
          }
     }
