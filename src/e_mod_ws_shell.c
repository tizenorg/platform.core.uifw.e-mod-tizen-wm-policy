...
static Eina_List *wssh_services = NULL;
...
_e_tizen_ws_shell_service_destroy(struct wl_resource *resource)
{
   ...
   wssh_services = eina_list_remove(wssh_services, service);
                                    ...
                                    const char *name)
{
   ...
   EINA_LIST_FOREACH(wssh_services, l, service)
     {
        if (!(e_util_strcmp(service->name, name)))
          {
   ...
   wssh_services = eina_list_append(wssh_services, service);
                          ...
                          uint32_t id)
{
   ...
   EINA_LIST_FOREACH(wssh_services, l, service)
     {
        tizen_ws_shell_send_service_register(res, service->name);
     }
}
...
e_mod_ws_shell_shutdown(void)
{
   ...
   EINA_LIST_FREE(wssh_services, service)
